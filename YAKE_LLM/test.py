...
parser.add_argument('--load_keys', default=True,
                    type=bool, help='Continue from clustering data')
parser.add_argument('--load_original_keys', default=False,
                    type=bool, help='Continue from extracting data')
parser.add_argument('--alpha', default=0.1, type=int, help='alpha')
...
def load_data(addr, args):
    ...
    # 加入 , dtype=object
    fact_clean = np.array(fact_clean, dtype=object)[random_idx]
    accu_clean = np.array(accu_clean, dtype=object)[random_idx]
    fact_original_new = []
    for i in random_idx:
...
def extract_keywords(addr, fact_original, accu_clean, train_split, num_select1, num_select2):
    # noinspection PyUnresolvedReferences
    import pke
    import spacy
    from yake_pke import YAKE
    ...
    def count_df(fact_accu, stop_words):
        def count_df_accu(text, stop_words):
            ...
            start_pos = 0
            max_len = 10 ** 6
            print(f"df: {new_text_len}")
            while new_text_len > max_len:
                print("======= cut text =======")
                print(f"start: {start_pos}")
                # set end position
                end_pos = start_pos + max_len - 1

                # add 10**6 text to texts
                texts.append(new_text[start_pos:end_pos])

                # get start position
                start_pos = end_pos

                # refresh left text
                new_text = new_text[start_pos:]
                new_text_len = len(new_text)

            if not texts:
                print("====== processing ======")
                print("empty!")
                # print(f"text: {new_text_len}")
                extractor.load_document(new_text, language='zh', spacy_model=nlp)
                extractor.candidate_selection(n=1, stoplist=stop_words)
                return extractor.candidate_dict()
            else:
                print("====== processing ======")
                print("not empty!")
                candidates_dict = Counter()
                for t in texts:
                    print(f"t: {len(t)}")
                    extractor.load_document(t, language='zh', spacy_model=nlp)
                    extractor.candidate_selection(n=1, stoplist=stop_words)
                    candidates_dict.update(extractor.candidate_dict())
    ...
    def extract_yake(text, stop_words, num_select, df, N):
        ...
        print(f'extract_yake: {len("".join(text))}')
        extractor.load_document("".join(text), language='zh', spacy_model=nlp)
        extractor.candidate_selection(n=1, stoplist=stop_words)
    ...
    if args.load_original_keys:
        df, df_last = count_df(fact_accu, stop_words)
        dump_keys(df, df_last)
        # Initialize an empty list to store keys
        all_keys = []

        # Extract keys from df where count > 10
        for key, count in df.items():
            if count > 10:
                all_keys.append(key)

        # Extract keys from df_last where count > 10
        for key, count in df_last.items():
            if count > 10:
                all_keys.append(key)
    else:
        print("extracting with LLM...")
        all_keys = load_dumped_keys()

    original_combine_key = extract_LLM(all_keys)
    original_combine_key = [key for key in original_combine_key if key in all_keys]
...
def dump_keys(df, df_last, threshold=10):
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        # Iterate through the items in the first Counter (df)
        for key, count in df.items():
            if count > threshold:
                f.write(f'{key}\n')  # Write the key to the file

        # Iterate through the items in the second Counter (df_last)
        for key, count in df_last.items():
            if count > threshold:
                f.write(f'{key}\n')  # Write the key to the file
...
def cluster_keywords(wv, original_combine_key, num_clusters):
    print('cluster keyword...')
    km_cluster = KMeans(n_clusters=num_clusters, init='k-means++', n_init=40, max_iter=300, tol=0.0001, verbose=0,
                        random_state=None, copy_x=True, algorithm='auto')

    print("===============original_combine_key===============")
    print(original_combine_key)
    print("===============original_combine_key===============")
    ...
    print()
    print("===============original_combine_key_new===============")
    print(original_combine_key_new)
...
def find_factors(fact_clean, accu_clean, accu_select, train_split, combine_key):
    ...
    # combine_key = combine_key_dic.values()
...
def dump_data(combine_key, accu_select, y, factor, idx, word_idx, word_key, random_idx, fact_original, fact_clean, accu_clean):
    print('dump data...')
    with open(addr + 'factor_name' + suffix + extract_way + '.txt', 'w') as f:
    ...
    with open(addr + 'factor' + suffix + extract_way + '.tsv', 'w') as f:
        for i in range(len(combine_key)):
            f.write('x' + str(i) + '\t')
        for i in range(len(accu_select)):
    ...
    with open(addr + 'data' + suffix + extract_way + '.pkl', 'wb') as f:
        pkl.dump(data_new, f)

    with open(addr + 'text' + suffix + extract_way + '.tsv', 'w') as f:
        for k in range(2):
            for i in range(len(word_idx[k])):
                f.write(str(idx[k][i]) + '\t'
...
def load_dumped_data(suffix):
    ...
    with open(addr + 'data' + suffix + extract_way + '.pkl', 'rb') as f:
        data = pkl.load(f)
    idx = data['idx']
...
def time_constraint(fact_clean, idx, combine_key, suffix):
                ...
                for j in range(len(fact_train[i])):
                    ...
                    for p in combine_key[k]:
                        ...
                        if d < 20:
                            ...
                            first_occur[i][k] = j
                            break
        np.save(addr + 'first_occur' + suffix + extract_way + '.npy', first_occur)
        return first_occur

    print('time constraint...')
    start_time = datetime.now()

    first_occur_file = addr + 'first_occur' + suffix + extract_way + '.npy'
    ...
    for i in range(len(combine_key)):
        for j in range(i + 1, len(combine_key)):
            # if i < len(first_occur[0]) and j < len(first_occur[0]):
    ...
    with open(addr + 'time_constraint' + suffix + extract_way + '.pkl', 'wb') as f:
...
def data_augmentation(suffix, accu_select, idx, word_idx, word_key, train_split):
    df = pd.read_csv(addr + 'factor' + suffix + extract_way + '.tsv', sep='\t')
    df_train = df[:train_split]
    df_test = df[train_split:]
    new_df_train = df_train.copy()

    text = pd.read_csv(addr + 'text' + suffix + extract_way + '.tsv', sep='\t')
    ...
    for i in y_cols:
        if accu_cnt[i] < min_limit:
            ...
            # 加入 , dtype=object
            new_word_idx[0].extend(list(np.array(word_idx[0], dtype=object)[idx_chosen]))
            new_word_key[0].extend(list(np.array(word_key[0], dtype=object)[idx_chosen]))
    ...
    new_df.to_csv(addr + 'factor_aug' + suffix + extract_way + '.tsv', sep='\t', index=0)
    new_text = pd.concat([new_text_train, text_test])
    new_text.to_csv(addr + 'text_aug' + suffix + extract_way + '.tsv', sep='\t', index=0)
...
def build_causal_graph(suffix, train_split, constraint, accu_select, graph_samples_num, augment=False):
    ...
    if augment:
        df = pd.read_csv(addr + 'factor_aug' + suffix + extract_way + '.tsv', sep='\t')
    else:
        df = pd.read_csv(addr + 'factor' + suffix + extract_way + '.tsv', sep='\t')
    df_train = df[:train_split]
    pc = pc()
    ...
    for k in range(graph_samples_num):
        ...
        G_samples.append(G)
        with open(addr + 'graph' + suffix + '_' + str(k) + extract_way + '.dot', 'w') as f:
            write_dot(G, f)

    with open(addr + 'graph_samples' + suffix + extract_way + '.pkl', 'wb') as f:
...
def causal_strength(suffix, train_split, G_samples, accu_select, sensitivity=False, augment=False):
        ...
        for k in range(len(outcomes)):
            for i in treatments[k]:
                model = CausalModel(
                    ...
                    treatment=i,
                    outcome=outcomes[k],
                    graph=addr + 'graph' + suffix + '_' + str(t) + extract_way + '.dot',
                    verbose=0
                )
    ...
    with open(addr + 'est' + suffix + extract_way + '.pkl', 'wb') as f:
...
def chain(word_idx, word_key, fact_clean, idx, G_samples, G_scores, num_clusters, accu_select):
    ...
    with open(addr + 'chains' + suffix + extract_way + '.pkl', 'wb') as f:
...
def prepare_nn_data(fact_clean, fact_original, idx, word_idx, word_key, train_split, est, chains, suffix, accu_select,
    ...
    # 加入 , dtype=object
    word_idx_train = np.array(word_idx[0], dtype=object)[idx_pos]
    word_idx_test = np.array(word_idx[1], dtype=object)
    word_key_train = np.array(word_key[0], dtype=object)[idx_pos]
    word_key_test = np.array(word_key[1], dtype=object)
    ...
    with open(addr + 'nn_data' + suffix + extract_way + '.pkl', 'wb') as f:
        pkl.dump(data, f)

    with open(addr + 'nn_data' + suffix + extract_way + '.txt', 'w') as f:
...
if not args.sensitivity:
    if not args.fix_graph:
        ...
        with open(addr + 'graph_samples' + suffix + extract_way + '.pkl', 'rb') as f:
